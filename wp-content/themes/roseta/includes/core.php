...
function roseta_title_and_description() {
	...
	if ( in_array( $options['theme_siteheader'], array( 'title', 'both', 'logo', 'empty' ) ) ) {
		...
		echo '<span> <a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( get_bloginfo( 'description' ) ) . '" rel="home">' . $wp_logo . '</a> </span>';
		echo '</' . $heading_tag . '>';
...
function roseta_copyright() {
    echo '<div id="site-copyright-p">' . do_shortcode( wp_kses_post( cryout_get_option( 'theme_copyright' ) ) ). '  |   ALL RIGHTS RESERVED</div>';
}
...
function roseta_get_layout_class() {
	...
	switch( $layout ) {
		...
		case '1c':
		default: return "full-page"; break;
	}
...
function roseta_master_hook() {
	...
	if ( $theme_interim_options['theme_comlabels'] ) {
	...
	/* if ( cryout_get_option( 'theme_socials_header' ) ) 		add_action( 'cryout_header_socials_hook', 'roseta_socials_menu_header', 30 ); */
...
function roseta_lpbox_width( $options = array() ) {
	...
	if ( $options['theme_lpboxlayout1'] == 1 ) { $totalwidth = 1920; }
	else { $totalwidth = $options['theme_sitewidth']; }
	$options['theme_lpboxwidth1'] = intval ( $totalwidth / $options['theme_lpboxrow1'] );

	if ( $options['theme_lpboxlayout2'] == 1 ) { $totalwidth = 1920; }
	else { $totalwidth = $options['theme_sitewidth']; }
...
// roseta ajax load more button alternative hook
